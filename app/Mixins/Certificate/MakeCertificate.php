...
class MakeCertificate
{
    ...
    private function makeImage($certificateTemplate, $body)
    {
        ...
        $lines = explode("\n", $body);
        $y = $certificateTemplate->position_y;

        foreach ($lines as $line) {
            $img->text($line, $img->width() / 2, $y, function ($font) use ($certificateTemplate) {
                ...
                $font->align('center');  // Menempatkan teks secara horizontal di tengah gambar
            });

            $y += $certificateTemplate->font_size + 5; // Sesuaikan jarak antar baris
        }
