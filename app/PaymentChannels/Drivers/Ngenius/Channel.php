...
class Channel extends BasePaymentChannel implements IChannel
{
    ...
    public function paymentRequest(Order $order)
    {
        ...
        $paymentCreateHeaders = ['Authorization: Bearer '.$access_token, 'Content-Type: application/vnd.ni-payment.v2+json', 'Accept: application/vnd.ni-payment.v2+json'];
            ...
            if (! empty($order)) {
                ...
                if (! empty($data['ref'])) {
                    ...
                    if ($paymentStatusResponse->_embedded->payment[0]->state == 'FAILED') {
                        $orderStatus = Order::$fail;
                    } elseif ($paymentStatusResponse->_embedded->payment[0]->state == 'CAPTURED') {
