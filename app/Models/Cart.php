    ...
    public static function getCartsTotalPrice($carts)
    {
        ...
        if (! empty($carts) and count($carts)) {
            foreach ($carts as $cart) {
                ...
                } elseif (! empty($cart->webinar_id) and ! empty($cart->webinar)) {
                    $totalPrice += $cart->webinar->price;
                } elseif (! empty($cart->bundle_id) and ! empty($cart->bundle)) {
                    $totalPrice += $cart->bundle->price;
                } elseif (! empty($cart->reserve_meeting_id) and ! empty($cart->reserveMeeting)) {
                    $totalPrice += $cart->reserveMeeting->paid_amount;
                } elseif (! empty($cart->product_order_id) and ! empty($cart->productOrder) and ! empty($cart->productOrder->product)) {
