...
class Webinar extends Model implements TranslatableContract
{
    ...
    public static $textLesson = 'text_lesson';
    ...
    public function bestTicketGuest($with_percent = false)
    {
        $ticketPercent = 0;
        $bestTicket = $this->price_guest;

        $activeSpecialOffer = $this->activeSpecialOffer();

        if ($activeSpecialOffer) {
            $bestTicket = $this->price_guest - ($this->price_guest * $activeSpecialOffer->percent / 100);
            $ticketPercent = $activeSpecialOffer->percent;
        } else {
            foreach ($this->tickets as $ticket) {

                if ($ticket->isValid()) {
                    $discount = $this->price_guest - ($this->price_guest * $ticket->discount / 100);
    ...
    public function getDiscountGuest($ticket = null, $user = null)
    {
        $activeSpecialOffer = $this->activeSpecialOffer();

        $discountOut = $activeSpecialOffer ? $this->price_guest * $activeSpecialOffer->percent / 100 : 0;

        if (! empty($user) and ! empty($user->getUserGroup()) and isset($user->getUserGroup()->discount) and $user->getUserGroup()->discount > 0) {
            $discountOut += $this->price_guest * $user->getUserGroup()->discount / 100;
        }

        if (! empty($ticket) and $ticket->isValid()) {
            $discountOut += $this->price_guest * $ticket->discount / 100;
        }
    ...
    {
        return url('/course/'.urlencode($this->slug));
    }

    public function getLearningPageUrl()
    {
        return url('/course/learning/'.urlencode($this->slug));
    }
    ...
    public function getPriceGuest()
    {
        $priceGuest = $this->price_guest;

        $specialOffer = $this->activeSpecialOffer();
        if (! empty($specialOffer)) {
            $priceGuest = $priceGuest - ($priceGuest * $specialOffer->percent / 100);
        }
    ...
    public function petaProvinsi()
    {
        return $this->belongsTo(PetaKabkota::class, 'province_code', 'mst_propinsi_id');
    }
    
    public function petaKabkota()
    {
        return $this->belongsTo(PetaKabkota::class, 'city_code', 'mst_kabupaten_id');
    } 

    public function user()
    {
        return $this->belongsTo(User::class, 'user_id'); // Sesuaikan kolom yang digunakan sebagai foreign key
    }
}
