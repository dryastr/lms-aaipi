    ...
    */
    ...
    protected function create(array $data)
    {
        ...
        $user = User::create([
            ...
            'upload_file' => isset($data['upload_file']) ? $data['upload_file'] : null,
            'upload_sertifikat' => isset($data['upload_sertifikat']) ? $data['upload_sertifikat'] : null,
            'status' => User::$pending,
            'access_content' => $accessContent,
            'password' => Hash::make($data['password']),
            'is_member_aaipi' => $data['is_member_aaipi'] ?? false,
            'affiliate' => $usersAffiliateStatus,
            'timezone' => $data['timezone'] ?? null,
            'created_at' => time(),
        ]);

        // if (! empty($data['upload_file']) && $data['upload_file'] instanceof UploadedFile) {
        //     $path = $data['upload_file']->store('upload/file', 'public');
        //     $user->update(['upload_file' => $path]);
        // }

        // if (! empty($data['upload_sertifikat']) && $data['upload_sertifikat'] instanceof UploadedFile) {
        //     $pathSertifikat = $data['upload_sertifikat']->store('upload/sertifikat', 'public');
        //     $user->update(['upload_sertifikat' => $pathSertifikat]);
        // }
    ...
    public function register(Request $request)
    {
        ...
        if ($checkConfirmed['status'] == 'send') {
            ...
            $toastData = [
                'title' => trans('panel.berhasil_registrasi'),
                'msg' => trans('panel.silahkan_cek_email'),
                'status' => 'success',
            ];

            return redirect('/verification')->with(['toast' => $toastData]);
