        ...
        if (! empty($verificationId)) {
            ...
            if (! empty($verification)) {
                ...
                if (! empty($user) and $user->status != User::$active) {
                    ...
                    // Mengambil bagian sebelum "@" untuk disamarkan
                    $emailParts = explode('@', $usernameValue);
                    $maskedUsername = substr($emailParts[0], 0, 2) . str_repeat('*', strlen($emailParts[0]) - 4) . substr($emailParts[0], -1);
