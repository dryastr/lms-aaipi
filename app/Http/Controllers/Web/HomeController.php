...
{
    protected function getTestimonialsCount()
    {
    ...
    public function index()
    {
        ...
        if (in_array(HomeSection::$instructors, $selectedSectionsName)) {
            $instructors = User::where('role_name', Role::$teacher)
                ->select('id', 'full_name', 'avatar', 'bio', 'certificate')
                ->where('status', 'active')
                ->where(function ($query) {
        ...
        if (in_array(HomeSection::$instructors, $selectedSectionsName)) {
            $instructorsCount = User::where('role_name', Role::$teacher)
                ->whereNotNull('certificate') // Memastikan certificate tidak null
                ->where('status', 'active')
                ->where(function ($query) {
                    $query->where('ban', false)
                        ->orWhere(function ($query) {
        ...
        if (in_array(HomeSection::$testimonials, $selectedSectionsName)) {
            ...
            $totalTestimonialsCount = HomeSection::where('name', HomeSection::$testimonials)->count();
        }

        // dd($selectedSectionsName);
        $adsBanners = IklanBanner::all();
        // dd($adsBanners);
        ...
        $firstSectionTestimonials = Testimonial::where('status', 'active')->limit(5)->get();
        $testimonials = Testimonial::where('status', 'active')->limit(10)->get();

        $totalTestimonialsCount = Testimonial::count();
        $displayedTestimonialsCount = min(5, $totalTestimonialsCount);
        $hiddenTestimonialsCount = max(0, $totalTestimonialsCount - 5);
        $hiddenTestimonialsCountFormatted = $this->format_number($hiddenTestimonialsCount, 1, 1);
        ...
        $data = [
            ...
            'displayedTestimonialsCount' => $displayedTestimonialsCount,
            'hiddenTestimonialsCount' => $hiddenTestimonialsCount,
            'firstSectionTestimonials' => $firstSectionTestimonials,
            'hiddenTestimonialsCountFormatted' => $hiddenTestimonialsCountFormatted,
    ...
    public function indexGridBestRate()
    {
        $homeSections = HomeSection::orderBy('order', 'asc')->get();
        $selectedSectionsName = $homeSections->pluck('name')->toArray();
        ...
        return view(getTemplate().'.includes.webinar.grid-card', $data);
    }
    ...
    public function format_number($number)
    {
        ...
        $input_count = substr_count($formattedNumber, ',');

        if ($input_count != '0') {
            if ($input_count == '1') {
                return substr($formattedNumber, 0, -4).'k';
            } elseif ($input_count == '2') {
                return substr($formattedNumber, 0, -8).'mil';
            } elseif ($input_count == '3') {
                return substr($formattedNumber, 0, -12).'bil';
            } else {
    ...
    {
        $skillfulTeachersCount = $this->format_number(User::where('role_name', Role::$teacher)
            ->where(function ($query) {
                $query->where('ban', false)
                    ->orWhere(function ($query) {
                        $query->whereNotNull('ban_end_at')
        ...
        $studentsCount = $this->format_number(User::where('role_name', Role::$user)
            ->where(function ($query) {
                $query->where('ban', false)
                    ->orWhere(function ($query) {
                        $query->whereNotNull('ban_end_at')
        ...
        $liveClassCount = $this->format_number(Webinar::where('type', 'webinar')
            ->where('status', 'active')
            ->count());

        $offlineCourseCount = $this->format_number(Webinar::where('status', 'active')
