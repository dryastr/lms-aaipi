...
class CartController extends Controller
{
        ...
        if (! empty($carts) and ! $carts->isEmpty()) {
            ...
            if (! empty($calculate)) {
                ...
                $data = [
                    ...
                    'user' => $user,
                    'carts' => $carts,
                    'totalPriceGuest' => $calculate['totalPriceGuest'],
                    'subTotal' => $calculate['sub_total'],
                    'subTotalPriceGuset' => $calculate['subTotalPriceGuset'],
        ...
        foreach ($carts as $cart) {
            if (! empty($cart->productOrder) and ! empty($cart->productOrder->product)) {
                ...
                if (! empty($product->delivery_fee)) {
                    ...
                    } elseif (empty($productFee[$product->creator_id])) {
                        $productFee[$product->creator_id] = $product->delivery_fee;
                    }
                }
            }
    ...
    private function calculatePrice($carts, $user, $discountCoupon = null)
    {
        ...
        foreach ($carts as $cart) {
            ...
            $subTotalPriceGuset += $webinar->price_guest ?? $webinar->price_guest ?? $orderPrices['sub_total'];
            // dd($subTotalPriceGuset);
        ...
        $subTotalWithoutDiscountGuest = $subTotalPriceGuset - $totalDiscount;
        $productDeliveryFee = $this->calculateProductDeliveryFee($carts);

        $total = $subTotalWithoutDiscount + $taxPrice + $productDeliveryFee;
        $totalPriceGuest = $subTotalWithoutDiscountGuest + $taxPrice + $productDeliveryFee;
        // dd($totalPriceGuest);

        if ($total < 0) {
            $total = 0;
        }
        if ($totalPriceGuest < 0) {
            $totalPriceGuest = 0;
        }
        ...
        return [
            ...
            'tax' => $tax,
            'totalPriceGuest' => round($totalPriceGuest, 2),
            'subTotalPriceGuset' => round($subTotalPriceGuset, 2),
                ...
                if (! empty($rules) and count($rules)) {
                    foreach ($rules as $rule) {
                        if (empty($rule->min_amount) or $rule->min_amount <= $subTotal) {
                            ...
                            } elseif ($rule->amount_type == 'percent') {
