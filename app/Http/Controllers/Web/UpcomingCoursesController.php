...
class UpcomingCoursesController extends Controller
{
    ...
    public function shareCount(Request $request, $slug)
    {
        $upcomingCourse = UpcomingCourse::where('slug', $slug)->firstOrFail();
        $upcomingCourse->increment('share_count');
        $upcomingCourse->save(); // Menyimpan perubahan ke database
