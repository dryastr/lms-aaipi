...
class UserController extends Controller
{
    ...
    public function instructors(Request $request, $is_export_excel = false)
    {
        ...
        // Mengambil pengguna dengan role "Anggota luar biasa"
        $query = User::where('role_name', '=', 'anggota_luar_biasa');

        $totalInstructors = deepClone($query)->count();
        $inactiveInstructors = deepClone($query)->where('status', 'inactive')->count();
        $banInstructors = deepClone($query)->where('ban', true)->whereNotNull('ban_end_at')->where('ban_end_at', '>', time())->count();

        $totalOrganizationsInstructors = User::where('role_name', 'Anggota luar biasa role')
            ->whereNotNull('organ_id')
            ->count();
    ...
    public function roleDpw(Request $request)
    {
        $this->authorize('admin_users_dpw');
    
        $dpwRoles = Role::where('name', 'dpw')->where('is_admin', true)->get();
    
        $dpwRoleIds = $dpwRoles->pluck('id')->toArray();
    
        $users = User::whereIn('role_id', $dpwRoleIds)
                    ->orderBy('created_at', 'desc')
                    ->paginate(10);
    
        $data = [
            'pageTitle' => trans('admin/main.dpw_list_title'),
            'users' => $users,
            'dpwRoles' => $dpwRoles,
        ];
        ...
        return view('admin.users.role_dpw', $data);
    }    
    ...
    public function create()
    {
        ...
        $provinces = PetaProvinsi::orderBy('province_code')->get();
        $cities = [];
    ...
    public function getCitiesByProvince(Request $request)
    {
        ...
        $cities = PetaKabKota::where('province_code', $provinceCode)->orderBy('city_code')->get();
    ...
    public function store(Request $request)
    {
        ...
        if (! empty($data['role_id'])) {
            ...
            if (! empty($role)) {
                ...
                $province_name = isset($data['province_name']) ? $data['province_name'] : null;
    ...
    public function edit(Request $request, $id)
    {
        ...
        $badges = Badge::all();
        // $citiess = RefCity::all();
        $citiess =PetaKabKota::orderBy('city_code')->get();
        $provincess = PetaProvinsi::orderBy('province_code')->get();
        // $provincess = RefProvince::all();
    ...
    public function update(Request $request, $id)
    {
        ...
        $user->province_code = $data['province_code'] ?? null;
        $user->city_code = $data['city_code'] ?? null;
    ...
    public function kehormatan()
    {
        // $groups = User::all();
        $groups = User::where('role_name', '=', 'angggota_kehormatan')->get();

        return view('admin.kehormatan.index', ['groups' => $groups]);
    ...
    public function biasa()
    {
        ...
        $groups = User::where('role_name', '=', 'anggota_biasa')->get();

        return view('admin.biasa.index', ['groups' => $groups]);
    }

    public function dpn()
    {
        // $groups = CrossCompetency::all();
        $groups = User::where('role_name', '=', 'dpn')->get();

        return view('admin.dpn.index', ['groups' => $groups]);
    }

    public function dpw()
    {
        // $groups = CrossCompetency::all();
        $groups = User::where('role_name', '=', 'dpw')->get();

        return view('admin.dpw.index', ['groups' => $groups]);
    }
}