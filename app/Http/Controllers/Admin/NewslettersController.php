...
class NewslettersController extends Controller
{
    ...
    private function handleSendToAllNewsletters($title, $description)
    {
        ...
        try {
            foreach ($newsletters as $newsletter) {
                \App\Jobs\SendEmail::dispatch($newsletter->email, ['title' => $title, 'message' => $description]);
    ...
    private function handleSendToCC($data)
    {
        ...
        try {
            \App\Jobs\SendEmail::dispatch($email, ['title' => $title, 'message' => $description, 'cc' => $ccEmails]);
            ...
            if (! empty($rows) and count($rows[0])) {
                foreach ($rows[0] as $row) {
                    if (! empty($row) and ! empty($row[0])) {
                        ...
                        \App\Jobs\SendEmail::dispatch($email, ['title' => $title, 'message' => $description]);
