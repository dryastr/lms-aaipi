...
class WebinarController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Webinar::where('webinars.type', $type)->with(['role', 'user']);
        ...
        $webinars = $query->when($province_code, function ($query) use ($province_code) {
            return $query->whereHas('teacher', function ($q) use ($province_code) {
                $q->where('province_code', $province_code);
            });
        })->paginate(8);
        ...
        $query = $this->filterWebinar($query, $request)
            ->with([
                'category',
                'teacher' => function ($qu) {
                    $qu->select('id', 'full_name', 'city_code', 'province_code');
                },
                'sales' => function ($query) {
                    $query->whereNull('refund_at');
                },
                'user' => function ($query) use ($province_code,) {
    ...
    public function create()
    {
        ...
        // $resources = new Webinar(); // Buat instans baru untuk mengakses relasi
        // $resources->filterOptions; // Akses relasi filterOptions
        // dd($resources->filterOptions); // Debug filterOptions
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'live_streaming_url' => ['nullable', 'string', 'regex:/^https:/'],
            'thumbnail' => 'required',
            'image_cover' => 'required',
            'description' => 'required|min:300',
            'teacher_id' => 'required|exists:users,id',
            'category_id' => 'required',
            'other_category_id' => 'required|exists:other_categories,id',
        ...
        $data['price_guest'] = ! empty($data['price_guest']) ? convertPriceToDefaultCurrency($data['price_guest']) : null;
        $data['organization_price'] = ! empty($data['organization_price']) ? convertPriceToDefaultCurrency($data['organization_price']) : null;

        $otherCategory = CrossCompetency::find($data['other_category_id']);
        if ($otherCategory) {
            $data['other_category_type'] = $otherCategory->types;
        }
        ...
        $webinar = Webinar::create([
            ...
            'is_live_streaming' => ! empty($data['is_live_streaming']) ? true : false,
            'live_streaming_url' => $data['live_streaming_url'] ? $data['live_streaming_url'] : null,
            ...
            'price' => $data['price'],
            'price_guest' => $data['price_guest'],
            'organization_price' => $data['organization_price'] ?? null,
            'points' => $data['points'] ?? null,
            'category_id' => $data['category_id'],
            'message_for_reviewer' => $data['message_for_reviewer'] ?? null,
            'status' => Webinar::$pending,
            'other_category_id' => $data['other_category_id'],
            'other_category_type' => $data['other_category_type'],
            'created_at' => time(),
            'updated_at' => time(),
        ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        if (! empty($data['other_category_id'])) {
            $otherCategory = CrossCompetency::find($data['other_category_id']);
            if ($otherCategory) {
                $data['other_category_type'] = $otherCategory->types;
            }
        ...
        $data['is_live_streaming'] = ! empty($data['is_live_streaming']) ? true : false;
        ...
        $webinar->update([
            ...
            'live_streaming_url' => $data['live_streaming_url'] ?? null,
