            ...
            <form action="{{ getAdminPanelUrl() }}/settings/main" method="post">
                ...
                @foreach(['instructors_count','students_count','courses_capacity','courses_count','meeting_count','product_count'] as $str)
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Cari" name="value[{{ $str }}]" value="{{ (!empty($organizationsSettings) and isset($organizationsSettings[$str])) ? $organizationsSettings[$str] : '' }}">
