            ...
            <form action="{{ getAdminPanelUrl() }}/settings/main" method="post">
                ...
                @foreach(['courses_capacity','courses_count','meeting_count','product_count'] as $str)
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Cari" name="value[{{ $str }}]" value="{{ (!empty($instructorsSettings) and isset($instructorsSettings[$str])) ? $instructorsSettings[$str] : '' }}">
                    </div>
                @endforeach
