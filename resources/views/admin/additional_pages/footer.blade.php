                                    ...
                                    <div class="tab-pane mt-3 fade {{ $loop->iteration == 1 ? 'show active' : '' }}" id="{{ $column }}" role="tabpanel" aria-labelledby="{{ $column }}-tab">
                                        <div class="">
                                            <form action="{{ getAdminPanelUrl() }}/additional_page/footer/store" method="post">
                                                ...
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <label>{{ trans('admin/main.content') }}<span class="text-danger">*</span></label>
                                                        <div class="form-text text-muted mb-10">
                                                            <span>Teks dibawah adalah data yang akan ditampilkan di footer halaman website</span>
                                                        </div>
                                                        <textarea type="text" name="value[{{ $column }}][value]" class="summernote form-control" value="{{ (!empty($value) and !empty($value[$column]) and !empty($value[$column]['value'])) ? $value[$column]['value'] : old('value') }}">{{ (!empty($value) and !empty($value[$column]) and !empty($value[$column]['value'])) ? $value[$column]['value'] : old('value') }}</textarea>
                                                    </div>
