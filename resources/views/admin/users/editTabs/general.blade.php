    ...
    <div class="row">
        <div class="col-12 col-md-6">
            <form action="{{ getAdminPanelUrl() }}/users/{{ $user->id .'/update' }}" method="Post">
                ...
                <div class="form-group">
                    <label>{{ trans('/public.provinsi') }}</label>
                    <select class="form-control select2 @error('role_id') is-invalid @enderror" id="province_name" name="province_code">
                        <option  value="" >{{ trans('/public.provinsi') }}</option>
                        @foreach ($provincess as $role)
                            <option value="{{ $role->province_code }}" {{ (!empty($user) and $user->province_code == $role->province_code) ? 'selected' :''}}>{{ $role->propinsi_name }}</option>
                        @endforeach
                    </select>
                    @error('propinsi_name')
                    <div class="invalid-feedback">
                ...
                <div class="form-group">
                    <label>{{ trans('/admin/main.kabupaten') }}<span class="text-danger">*</span></label>
                    <select class="form-control select2 @error('city_code') is-invalid @enderror" id="city_code" name="city_code">
                    <option>{{ trans('admin/main.kabupaten') }}</option>
                        @foreach ($citiess as $role)
                            <option value="{{ $role->city_code }}" {{ (!empty($user) and $user->city_code == $role->city_code) ? 'selected' :''}}>{{ $role->kabupaten_name }}</option>
                        @endforeach
                    </select>
                    @error('city_name')
                    <div class="invalid-feedback">
                ...
                {{-- <div class="form-group">
                    <label>{{ trans('admin/main.password') }}<span class="text-danger">*</span></label>
                    <input type="password" id="password" name="password" value="{{$user->password}}" class="form-control @error('password') is-invalid @enderror" onkeyup="checkPasswordStrength()" maxlength="20"/>
