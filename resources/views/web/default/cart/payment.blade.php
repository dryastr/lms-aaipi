...
@section('content')
    <section class="cart-banner position-relative text-center">
        ...
        @if (!$user->is_member_aaipi)
            <span class="payment-hint font-20 text-white d-block">{{ handlePrice($totalPriceGuest) . ' ' .  trans('cart.for_items',['count' => $count]) }}</span>
        @else
    ...
    <section class="container mt-45">
        ...
        <form action="/payments/payment-request" method="post" class=" mt-25">
            ...
            <div class="d-flex align-items-center justify-content-between mt-45">
                @if (isset($user))
                @if (!$user->is_member_aaipi)
                <span class="font-16 font-weight-500 text-gray">{{ trans('financial.total_amount') }} {{ handlePrice($totalPriceGuest) }}</span>                    
                @else
