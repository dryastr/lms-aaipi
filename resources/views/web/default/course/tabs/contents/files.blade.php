            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                ...
                <div class="">
                    @if(!empty($checkSequenceContent) and $sequenceContentHasError)
                        ...
                        @elseif($file->accessibility == 'paid' && $user && $course->price_guest == 0 && !$hasBought)
                    ...
                    @else
                        ...
                        @else
                            ...
                            @elseif($file->isVideo())
                            <button type="button" data-id="{{ $file->id }}" data-title="{{ $file->title }}" class="course-content-btns {{ (empty($user) ? 'btn btn-sm btn-gray disabled not-login-toast' : 'btn btn-sm btn-primary js-play-video') }}">
                                {{ trans('public.play') }}
                            </button>
                            {{-- @elseif($file->isVideo() && $course->price_guest == 0)
                                  <button type="button" 
                                  data-id="{{ $file->id }}" 
                                  data-title="{{ $file->title }}" 
                                  class="course-content-btns {{ (empty($user) ? 'btn btn-sm btn-gray disabled not-login-toast' : 'btn btn-sm btn-primary js-play-video') }}">
                              {{ trans('public.play') }}
                          </button> --}}
                            @else
...
<style>
    ...
    color: #000000; /* Warna teks yang cocok dengan latar belakang tombol */
    cursor: not-allowed; /* Mengubah kursor menjadi tidak diperbolehkan */
}
</style>