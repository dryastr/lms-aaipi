<div class="mt-35">
    <h2 class="section-title after-line"><span class="mr-5">{{ $comments->count() }}</span> {{ trans('panel.comments') }}</h2>
            ...
            <div class="comments-card shadow-lg rounded-sm border px-20 py-15 mt-30" data-address="/comments/{{ $comment->id }}/reply" data-csrf="{{ csrf_token() }}" data-id="{{ $comment->id }}">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="d-flex align-items-center">
                        <span class="font-12 text-gray mr-10">
                            {{-- {{ dateTimeFormat($comment->created_at, 'j M Y | H:i') }} --}}
                            {{-- {{ \Carbon\Carbon::parse($comment->created_at)->translatedFormat('j M Y | H:i') }} --}}
                            {{ \Carbon\Carbon::parse($comment->created_at)->locale('id')->translatedFormat('j F Y | H:i') }}
                        </span>
